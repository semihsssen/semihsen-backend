...
function uploadToCloudinary(buffer, folder, opts = {}) {
  return new Promise((resolve, reject) => {
    cloudinary.uploader.upload_stream({ folder, ...opts }, (err, result) => err ? reject(err) : resolve(result)).end(buffer);
  });
}
...
app.post('/api/upload/hero', adminAuth, upload.single('image'), async (req, res) => {
  ...
  } catch (e) { console.error('Hero error:', e.message); res.status(500).json({ error: e.message }); }
});
...
app.post('/api/upload/resume', adminAuth, upload.single('image'), async (req, res) => {
  try {
    ...
    saveDB(db); res.json({ ok: true, url: result.secure_url });
  } catch (e) { console.error('Resume error:', e.message); res.status(500).json({ error: e.message }); }
});

app.post('/api/upload/project/:key/slide', adminAuth, upload.single('image'), async (req, res) => {
  try {
    const db = getDB(); const proj = db.projects[req.params.key];
    if (!proj) return res.status(404).json({ error: 'Not found' });
    const result = await uploadToCloudinary(req.file.buffer, `semihsen/projects/${req.params.key}/slides`);
  ...
  } catch (e) { console.error('Slide error:', e.message); res.status(500).json({ error: e.message }); }
});
...
app.post('/api/upload/project/:key/grid', adminAuth, upload.single('image'), async (req, res) => {
  try {
    const db = getDB(); const proj = db.projects[req.params.key];
    if (!proj) return res.status(404).json({ error: 'Not found' });
    const result = await uploadToCloudinary(req.file.buffer, `semihsen/projects/${req.params.key}/grid`);
    if (proj.grid.length >= 4) { await cloudinary.uploader.destroy(proj.grid[0].public_id).catch(() => {}); proj.grid.shift(); }
    proj.grid.push({ url: result.secure_url, public_id: result.public_id });
    saveDB(db); res.json({ ok: true, url: result.secure_url, grid: proj.grid });
  } catch (e) { console.error('Grid error:', e.message); res.status(500).json({ error: e.message }); }
});
...
app.delete('/api/project/:key/slide/:idx', adminAuth, async (req, res) => {
  try {
    const db = getDB(); const proj = db.projects[req.params.key]; const idx = parseInt(req.params.idx);
...
app.post('/api/upload/personal/:num/slide', adminAuth, upload.single('image'), async (req, res) => {
  try {
    ...
    proj.slides.push({ url: result.secure_url, public_id: result.public_id });
    saveDB(db); res.json({ ok: true, url: result.secure_url, slides: proj.slides });
  } catch (e) { console.error('Personal error:', e.message); res.status(500).json({ error: e.message }); }
});
